...
import com.example.vocabmate.RetrofitClient;
import com.example.vocabmate.Service.VocabApiInterface;
...
public class VocabularyTestActivity extends AppCompatActivity {
    ...
    private void fetchVocabData() {
        VocabApiInterface vocabApi = RetrofitClient.getClient().create(VocabApiInterface.class);

        vocabApi.getVocabsByTopic(1).enqueue(new Callback<List<Vocab>>() {
            @Override
